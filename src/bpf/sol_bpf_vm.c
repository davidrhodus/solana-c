...
sol_bpf_memory_translate(
    ...
    for (size_t i = 0; i < mem->region_count; i++) {
        ...
        if (write && !region->writable) {
            return NULL; /* Write to read-only region */
        }

        if (region->kind == SOL_BPF_REGION_GAPPED) {
            ...
            uint64_t elem = offset / stride;
            uint64_t within = offset % stride;
...
sol_bpf_vm_execute(sol_bpf_vm_t* vm) {
    ...
    vm->state = SOL_BPF_STATE_RUNNING;
    vm->error = SOL_BPF_OK;
    vm->insn_count = 0;
    ...
    bool v2_pqr = sol_sbpf_enable_pqr(sbpf_ver);
    (void)sol_sbpf_explicit_sign_ext(sbpf_ver); /* sign/zero extension is now unconditional per Agave rbpf 0.8.3 */
...
#define TRACE_RING_MASK (TRACE_RING_SIZE - 1)
    ...
    while (vm->state == SOL_BPF_STATE_RUNNING) {
        /* Check compute budget */
        if (vm->compute_units_used >= vm->compute_units) {
        ...
        const sol_bpf_insn_t* insn = &insns[vm->pc];
        ...
        if (__builtin_expect(vm->trace, 0)) {
            sol_log_info("BPF_TRACE: pc=%lu op=0x%02x dst=r%u src=r%u off=%d imm=%d "
                         ...
                         (unsigned long)(vm->pc - 1), insn->opcode, dst, src, (int)off, (int)imm,
        ...
        case SOL_BPF_CLASS_ALU64: {
            ...
            if (__builtin_expect(v2_mem_classes, 0) &&
                ...
                uint64_t addr = vm->reg[dst] + off;
                uint8_t* host_addr = sol_bpf_memory_translate(&vm->memory, addr, st_len, true);
                if (host_addr == NULL) {
                ...
                uint64_t val = use_imm ? (uint64_t)(int64_t)imm : vm->reg[src];
                switch (st_len) {
            ...
            uint64_t src_val = use_imm ? (uint64_t)(int64_t)imm : vm->reg[src];
            ...
            case SOL_BPF_ALU_SUB:
                if (__builtin_expect(sol_sbpf_swap_sub_operands(sbpf_ver), 0) && use_imm)
                    vm->reg[dst] = src_val - vm->reg[dst];
                else
                    vm->reg[dst] -= src_val;
                break;
            ...
            case SOL_BPF_ALU_ARSH:
                vm->reg[dst] = (uint64_t)((int64_t)vm->reg[dst] >> (src_val & 63));
                break;
            case SOL_BPF_ALU_HOR:
                ...
                vm->reg[dst] |= ((uint64_t)(uint32_t)imm << 32);
                break;
            default:
        ...
        case SOL_BPF_CLASS_ALU: {
            ...
            if (__builtin_expect(v2_mem_classes, 0) && !use_imm &&
                ...
                uint64_t addr = vm->reg[src] + off;
                uint8_t* host_addr = sol_bpf_memory_translate(&vm->memory, addr, ld_len, false);
                if (host_addr == NULL) {
                ...
                switch (ld_len) {
                case 1: vm->reg[dst] = *(uint8_t*)host_addr;  break;
                case 2: vm->reg[dst] = *(uint16_t*)host_addr; break;
                case 4: vm->reg[dst] = *(uint32_t*)host_addr; break;
                case 8: vm->reg[dst] = *(uint64_t*)host_addr; break;
                }
                break;
            }

            uint32_t dst_val = (uint32_t)vm->reg[dst];
            uint32_t src_val = use_imm ? (uint32_t)imm : (uint32_t)vm->reg[src];
            ...
            bool sign_ext_arith = !sol_sbpf_explicit_sign_ext(sbpf_ver);
            switch (op_code) {
            case SOL_BPF_ALU_ADD:
                if (sign_ext_arith) {
                    vm->reg[dst] = (uint64_t)(int64_t)(int32_t)(dst_val + src_val);
                } else {
                    vm->reg[dst] = (uint64_t)(uint32_t)(dst_val + src_val);
                }
                break;
            case SOL_BPF_ALU_SUB:
                if (__builtin_expect(sol_sbpf_swap_sub_operands(sbpf_ver), 0) && use_imm) {
                    if (sign_ext_arith) {
                        vm->reg[dst] = (uint64_t)(int64_t)(int32_t)(src_val - dst_val);
                    } else {
                        vm->reg[dst] = (uint64_t)(uint32_t)(src_val - dst_val);
                    }
                } else {
                    if (sign_ext_arith) {
                        vm->reg[dst] = (uint64_t)(int64_t)(int32_t)(dst_val - src_val);
            ...
            case SOL_BPF_ALU_DIV:
                ...
                vm->reg[dst] = (uint64_t)(dst_val / src_val);
                break;
            case SOL_BPF_ALU_OR:
                vm->reg[dst] = (uint64_t)(dst_val | src_val);
                break;
            case SOL_BPF_ALU_AND:
                ...
                break;
            case SOL_BPF_ALU_LSH:
                vm->reg[dst] = (uint64_t)(dst_val << (src_val & 31));
                break;
            case SOL_BPF_ALU_RSH:
                vm->reg[dst] = (uint64_t)(dst_val >> (src_val & 31));
                break;
            case SOL_BPF_ALU_NEG:
                ...
                vm->reg[dst] = (uint64_t)(uint32_t)(-(int32_t)dst_val);
                break;
            case SOL_BPF_ALU_MOD:
                if (src_val == 0) {
                    sol_log_info("BPF_DIAG: ALU32 MOD div0 pc=%lu opcode=0x%02x v2_mem=%d use_imm=%d sbpf_ver=%d "
                                 "dst=r%u(0x%lx) src=r%u(0x%lx)",
                                 (unsigned long)(vm->pc - 1), insn->opcode, (int)v2_mem_classes,
                                 (int)use_imm, (int)sbpf_ver,
                                 dst, (unsigned long)vm->reg[dst], src, (unsigned long)vm->reg[src]);
                    vm->error = SOL_BPF_ERR_DIVIDE_BY_ZERO;
                    vm->state = SOL_BPF_STATE_ERROR;
                    goto done;
                }
            ...
            case SOL_BPF_ALU_XOR:
                ...
                break;
            case SOL_BPF_ALU_MOV:
                if (!use_imm && sol_sbpf_explicit_sign_ext(sbpf_ver)) {
                    /* V2+ MOV32_REG: sign-extend (Agave: self.reg[src] as i32 as i64 as u64) */
                    vm->reg[dst] = (uint64_t)(int64_t)(int32_t)src_val;
                } else {
                    /* V0/V1 MOV32_REG or any MOV32_IMM: zero-extend */
                    vm->reg[dst] = (uint64_t)src_val;
                }
                break;
            case SOL_BPF_ALU_ARSH:
                vm->reg[dst] = (uint64_t)(uint32_t)((int32_t)dst_val >> (src_val & 31));
                break;
            case SOL_BPF_ALU_END:
                if (use_imm) {
                    ...
                    if (imm == 16) {
                        vm->reg[dst] = (uint16_t)vm->reg[dst];
                    } else if (imm == 32) {
                        vm->reg[dst] = (uint32_t)vm->reg[dst];
                    } else if (imm == 64) {
                    ...
                    if (imm == 16) {
                        vm->reg[dst] = __builtin_bswap16((uint16_t)vm->reg[dst]);
                    } else if (imm == 32) {
                        vm->reg[dst] = __builtin_bswap32((uint32_t)vm->reg[dst]);
                    } else if (imm == 64) {
                        vm->reg[dst] = __builtin_bswap64(vm->reg[dst]);
                    }
                }
                break;
            default:
        ...
        case SOL_BPF_CLASS_JMP32: /* == SOL_BPF_CLASS_PQR (0x06) */ {
            /* In SBPFv2+, class 0x06 is PQR, not JMP32 */
            if (op_class == SOL_BPF_CLASS_JMP32 && v2_pqr) {
                goto handle_pqr;
            }
            uint64_t dst_val = vm->reg[dst];
            uint64_t src_val = use_imm ? (uint64_t)(int64_t)imm : vm->reg[src];
            ...
            case SOL_BPF_JMP_CALL:
                if (!use_imm) {
                    ...
                    uint32_t callx_reg = sol_sbpf_callx_uses_src(sbpf_ver) ? src : (uint32_t)imm;
                    ...
                    uint64_t target_addr = vm->reg[callx_reg];
                    ...
                    if (target_addr < text_start ||
                        ...
                        sol_log_debug("SBF callx target outside text: pc=%lu reg=%u target=0x%lx text=[0x%lx..0x%lx)",
                                      (unsigned long)(vm->pc ? (vm->pc - 1u) : 0u),
                                      (unsigned)callx_reg,
                    ...
                    frame->saved_regs[0] = vm->reg[6];
                    frame->saved_regs[1] = vm->reg[7];
                    frame->saved_regs[2] = vm->reg[8];
                    frame->saved_regs[3] = vm->reg[9];
                    frame->return_pc = vm->pc;
                    frame->frame_ptr = vm->reg[10];
                    ...
                    if (!sol_sbpf_dynamic_stack_frames(sbpf_ver)) {
                ...
                if (src == 0) {
                    ...
                    if (syscall == NULL) {
                        sol_log_error("BPF: unknown syscall hash=0x%08x cu_used=%lu",
                                      (unsigned)imm, (unsigned long)vm->compute_units_used);
                        vm->error = SOL_BPF_ERR_UNKNOWN_SYSCALL;
                        vm->state = SOL_BPF_STATE_ERROR;
                        goto done;
                    }

                    vm->reg[0] = syscall->handler(
                        vm,
                        vm->reg[1], vm->reg[2], vm->reg[3],
                        vm->reg[4], vm->reg[5]
                    );
                    break;
                }
                ...
                int64_t target_pc = (int64_t)vm->pc + (int64_t)imm;
                ...
                /* Jump to function */
                vm->pc = (uint64_t)target_pc;
                break;
            case SOL_BPF_JMP_EXIT:
                if (vm->call_depth == 0) {
                    /* Exit program */
                    if (vm->reg[0] >= 0x100000000ULL) {
                        sol_log_info("BPF EXIT_DIAG: r0=0x%lx r1=0x%lx r2=0x%lx pc=%lu cu=%lu",
                                     (unsigned long)vm->reg[0], (unsigned long)vm->reg[1],
                                     (unsigned long)vm->reg[2], (unsigned long)vm->pc,
                                     (unsigned long)vm->compute_units_used);
                    }
                    vm->return_value = vm->reg[0];
                    vm->state = SOL_BPF_STATE_STOPPED;
                    goto done;
                } else {
                    /* Return from function call */
                    sol_bpf_frame_t* ret_frame = &vm->call_stack[--vm->call_depth];
                    vm->reg[6] = ret_frame->saved_regs[0];
                    vm->reg[7] = ret_frame->saved_regs[1];
                    vm->reg[8] = ret_frame->saved_regs[2];
                    vm->reg[9] = ret_frame->saved_regs[3];
                    vm->pc = ret_frame->return_pc;
                    vm->reg[10] = ret_frame->frame_ptr;
                }
                break;
            default:
                vm->error = SOL_BPF_ERR_INVALID_INSN;
        ...
        case SOL_BPF_CLASS_LD: {
            ...
            if (mode == SOL_BPF_MODE_IMM && size == SOL_BPF_SIZE_DW) {
                ...
                const sol_bpf_insn_t* insn2 = &insns[vm->pc++];
                vm->insn_count++;
        ...
        case SOL_BPF_CLASS_LDX: {
            ...
            uint8_t* host_addr = sol_bpf_memory_translate(&vm->memory, addr, len, false);
            if (host_addr == NULL) {
            ...
            switch (size) {
            case SOL_BPF_SIZE_B:
                vm->reg[dst] = *(uint8_t*)host_addr;
                break;
            case SOL_BPF_SIZE_H:
                vm->reg[dst] = *(uint16_t*)host_addr;
                break;
            case SOL_BPF_SIZE_W:
                vm->reg[dst] = *(uint32_t*)host_addr;
                break;
            case SOL_BPF_SIZE_DW:
                vm->reg[dst] = *(uint64_t*)host_addr;
                break;
            }

            if (__builtin_expect(vm->trace, 0)) {
                sol_log_info("BPF_TRACE_LDX: vaddr=0x%lx len=%zu val=0x%lx -> r%u",
                             (unsigned long)addr, len, (unsigned long)vm->reg[dst], dst);
            }
            break;
        }
        ...
        case SOL_BPF_CLASS_ST: {
            ...
            uint8_t* host_addr = sol_bpf_memory_translate(&vm->memory, addr, len, true);
            if (host_addr == NULL) {
        ...
        case SOL_BPF_CLASS_STX: {
            ...
            switch (size) {
            case SOL_BPF_SIZE_B:
                *(uint8_t*)host_addr = (uint8_t)vm->reg[src];
                break;
            case SOL_BPF_SIZE_H:
                *(uint16_t*)host_addr = (uint16_t)vm->reg[src];
                break;
            case SOL_BPF_SIZE_W:
                *(uint32_t*)host_addr = (uint32_t)vm->reg[src];
                break;
            case SOL_BPF_SIZE_DW:
                *(uint64_t*)host_addr = vm->reg[src];
                break;
            }
            break;
        }
        ...
        handle_pqr: {
            ...
            case SOL_BPF_PQR_LMUL: {
                uint64_t pqr_src = use_imm ? (uint64_t)(int64_t)imm : vm->reg[src];
                if (pqr_64)
                    vm->reg[dst] *= pqr_src;
                else {
                    uint32_t r = (uint32_t)vm->reg[dst] * (uint32_t)pqr_src;
                    vm->reg[dst] = (uint64_t)r; /* zero-extend (rbpf: wrapping_mul as u64) */
                }
                break;
            }
            case SOL_BPF_PQR_UHMUL: {
                /* Unsigned: zero-extend immediate */
                uint64_t pqr_src = use_imm ? (uint64_t)(uint32_t)imm : vm->reg[src];
                if (pqr_64)
                    vm->reg[dst] = (uint64_t)((__uint128_t)vm->reg[dst] * (__uint128_t)pqr_src >> 64);
                else {
                    uint32_t r = (uint32_t)(((uint64_t)(uint32_t)vm->reg[dst] * (uint64_t)(uint32_t)pqr_src) >> 32);
                    vm->reg[dst] = (uint64_t)r; /* zero-extend */
                }
                break;
            }
            case SOL_BPF_PQR_UDIV: {
                ...
                uint64_t pqr_src = use_imm ? (uint64_t)(uint32_t)imm : vm->reg[src];
                if (pqr_64) {
                    if (pqr_src == 0) { vm->error = SOL_BPF_ERR_DIVIDE_BY_ZERO; vm->state = SOL_BPF_STATE_ERROR; goto done; }
                    vm->reg[dst] /= pqr_src;
                } else {
                    uint32_t d = (uint32_t)pqr_src;
                    if (d == 0) { vm->error = SOL_BPF_ERR_DIVIDE_BY_ZERO; vm->state = SOL_BPF_STATE_ERROR; goto done; }
                    vm->reg[dst] = (uint64_t)((uint32_t)vm->reg[dst] / d); /* zero-extend */
                }
                break;
            }
            ...
            case SOL_BPF_PQR_SHMUL: {
                uint64_t pqr_src = use_imm ? (uint64_t)(int64_t)imm : vm->reg[src];
                if (pqr_64)
                    vm->reg[dst] = (uint64_t)(((__int128)(int64_t)vm->reg[dst] * (__int128)(int64_t)pqr_src) >> 64);
                else {
                    uint32_t r = (uint32_t)((int64_t)(int32_t)(uint32_t)vm->reg[dst] * (int64_t)(int32_t)(uint32_t)pqr_src >> 32);
                    vm->reg[dst] = (uint64_t)(int64_t)(int32_t)r; /* sign-extend for SHMUL */
                }
                break;
            }
            case SOL_BPF_PQR_SDIV: {
                uint64_t pqr_src = use_imm ? (uint64_t)(int64_t)imm : vm->reg[src];
                if (pqr_64) {
                    int64_t a = (int64_t)vm->reg[dst], b = (int64_t)pqr_src;
                    if (b == 0) { vm->error = SOL_BPF_ERR_DIVIDE_BY_ZERO; vm->state = SOL_BPF_STATE_ERROR; goto done; }
                    if (a == INT64_MIN && b == -1) { vm->error = SOL_BPF_ERR_DIVIDE_OVERFLOW; vm->state = SOL_BPF_STATE_ERROR; goto done; }
                    vm->reg[dst] = (uint64_t)(a / b);
                } else {
                    int32_t a = (int32_t)(uint32_t)vm->reg[dst], b = (int32_t)(uint32_t)pqr_src;
                    if (b == 0) { vm->error = SOL_BPF_ERR_DIVIDE_BY_ZERO; vm->state = SOL_BPF_STATE_ERROR; goto done; }
                    if (a == INT32_MIN && b == -1) { vm->error = SOL_BPF_ERR_DIVIDE_OVERFLOW; vm->state = SOL_BPF_STATE_ERROR; goto done; }
                    vm->reg[dst] = (uint64_t)(uint32_t)(a / b); /* zero-extend (rbpf: as u32 as u64) */
                }
                break;
            }
