...
- Replay scheduler parent checks are lag-aware: TVU bounds `sol_replay_parent_ready()` probes per loop and reduces/skips them under backlog to prioritize replay throughput. Tune with `SOL_TVU_REPLAY_PARENT_PROBE_LIMIT` (default `64`), `SOL_TVU_REPLAY_PARENT_PROBE_HIGH_LAG` (default `256`), and `SOL_TVU_REPLAY_PARENT_PROBE_SEVERE_LAG` (default `1024`).
- Replay candidate selection is frontier-aware: TVU can bound how far ahead of `highest_replayed` COMPLETE slots are eligible for replay, reducing far-ahead replay-failure storms during catchup. Tune with `SOL_TVU_MAX_REPLAY_AHEAD_SLOTS` (default `1024`), `SOL_TVU_MAX_REPLAY_AHEAD_HIGH_LAG`/`SOL_TVU_MAX_REPLAY_AHEAD_HIGH_SLOTS` (defaults `512`/`64`), `SOL_TVU_MAX_REPLAY_AHEAD_SEVERE_LAG`/`SOL_TVU_MAX_REPLAY_AHEAD_SEVERE_SLOTS` (defaults `1024`/`24`), dead-primary tightening `SOL_TVU_PRIMARY_DEAD_REPLAY_AHEAD_SLOTS` (default `8`), and primary INCOMPLETE retry interval `SOL_TVU_PRIMARY_INCOMPLETE_RETRY_MS` (default `750`).
- TVU far-ahead shred intake is lag-aware: when `blockstore_highest - highest_replayed` grows, the max shred-ahead window tightens to keep queue/repair pressure near replay-critical slots. Base window: `SOL_TVU_MAX_SHRED_AHEAD_SLOTS` (default `4096`), high tier: `SOL_TVU_MAX_SHRED_AHEAD_HIGH_LAG`/`SOL_TVU_MAX_SHRED_AHEAD_HIGH_SLOTS` (defaults `2048`/`1024`), severe tier: `SOL_TVU_MAX_SHRED_AHEAD_SEVERE_LAG`/`SOL_TVU_MAX_SHRED_AHEAD_SEVERE_SLOTS` (defaults `8192`/`512`).
- Repair is primary-stall aware: when the next replay slot stops progressing, TVU enters a stricter primary window and prunes far-ahead pending repairs more aggressively so bandwidth stays focused on the blocked slot.
- Dead-primary duplicate repair is more aggressive by default and tunable: `SOL_TVU_DEAD_PRIMARY_DUPLICATE_FANOUT` (default `48`), `SOL_TVU_DEAD_PRIMARY_HIGHEST_FANOUT` (default `32`), `SOL_TVU_DEAD_PRIMARY_ORPHAN_FANOUT` (default `4`), and `SOL_TVU_DEAD_PRIMARY_ANCESTOR_FANOUT` (default `4`).
- Repair peer selection now favors higher diversity under retries/fanout (larger candidate pool and per-request peer rotation) to reduce long-tail stalls on small missing-shred gaps.
...
- Runtime RPC backpressure is enabled by default for combined RPC+voting nodes: when `blockstore_highest - highest_replayed` grows, RPC is throttled automatically to protect replay/voting (`SOL_RPC_BACKPRESSURE_*` envs: `HIGH_SLOTS`, `SEVERE_SLOTS`, `CLEAR_SLOTS`, `HIGH_RPS`, `SEVERE_RPS`, `HIGH_MAX_CONN`, `SEVERE_MAX_CONN`; set `SOL_RPC_BACKPRESSURE=0` to disable). Backpressure also supports lag-growth adaptation (`SOL_RPC_BACKPRESSURE_ADAPTIVE_GROWTH=1`, `SOL_RPC_BACKPRESSURE_HIGH_GROWTH_SLOTS`, `SOL_RPC_BACKPRESSURE_SEVERE_GROWTH_SLOTS`) so RPC sheds earlier when replay lag is accelerating. In backpressure mode, expensive methods are shed starting at `SOL_RPC_BACKPRESSURE_REJECT_MODE` (`2` by default = severe only). Per-method drop counters are exported via Prometheus metric `solana_rpc_backpressure_dropped_total{method=...}`.
- The combined mainnet profile script (`scripts/run-mainnet-rpc-voting.sh`) defaults to a replay-protective shape on large-core hosts: `SOL_REPLAY_VERIFY_WORKERS=24`, `SOL_REPLAY_VERIFY_WAIT_BUDGET_MS=256`, `SOL_TX_WORKERS=64`, `SOL_TX_PER_WORKER=8`, `SOL_TX_POOL_SHARDS=4`, `SOL_TX_DAG_POP_BATCH=1`, `SOL_TX_POOL_REPLAY_QUEUE_WAIT_LONG_BUDGET_MS=512`, and `SOL_TX_STATUS_BATCH_RECORD=0` (when tx index is disabled). TVU/repair canary defaults are tightened for backlog control (`SOL_TVU_MAX_SHRED_AHEAD_*`, `SOL_TVU_MAX_REPLAY_AHEAD_*`, `SOL_TVU_PRIMARY_DEAD_REPLAY_AHEAD_SLOTS=4`, `SOL_TVU_PRIMARY_INCOMPLETE_RETRY_MS=500`, `SOL_TVU_DEAD_PRIMARY_*_FANOUT`, `SOL_REPAIR_REQUEST_TIMEOUT_MS=120`, `SOL_REPAIR_MAX_PENDING=16384`, `SOL_REPAIR_MAX_RETRIES=6`). BPF prewarm defaults: `SOL_REPLAY_PREWARM_BPF_PROGRAMS=1`, `SOL_REPLAY_PREWARM_MAX_PROGRAMS=16384`, `SOL_REPLAY_PREWARM_MAX_VARIANTS=64`, with `SOL_BPF_PROG_CACHE_ENTRIES=131072`.
- Bank-forks capacity defaults to `4096` banks and can be tuned with `SOL_BANK_FORKS_MAX_BANKS`.
- The replay stage can auto-prune on bank-forks saturation (`SOL_REPLAY_PRUNE_ON_FULL=1`, default on) and retry insert immediately; tune with `SOL_REPLAY_PRUNE_ON_FULL_WINDOW`, `SOL_REPLAY_PRUNE_ON_FULL_COOLDOWN_MS`, and `SOL_REPLAY_PRUNE_ON_FULL_SEARCH_LIMIT`.
- Auto-root baseline mode applies in no-voting/fast-replay. Emergency auto-root can also engage during voting catchup when root progression stalls and forks approach capacity (`SOL_AUTO_ROOT_EMERGENCY=1`, `SOL_AUTO_ROOT_EMERGENCY_STALL_MS`, `SOL_AUTO_ROOT_EMERGENCY_LAG_SLOTS`, `SOL_AUTO_ROOT_EMERGENCY_HEADROOM`).
